...
const LOG_DIR = path$n.join(require$$1.app.getPath("userData"), "logs");
...
function ensureLogDir() {
  ...
  try {
    if (!fs$j.existsSync(LOG_DIR)) {
      fs$j.mkdirSync(LOG_DIR, { recursive: true });
    }
    _logFile = path$n.join(LOG_DIR, `app-${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.log`);
    _initialized = true;
  } catch {
  }
}
function rotateLogs() {
  try {
    if (!_logFile || !fs$j.existsSync(_logFile)) return;
    const stats = fs$j.statSync(_logFile);
    if (stats.size > MAX_LOG_SIZE_MB * 1024 * 1024) {
      const timestamp2 = Date.now();
      fs$j.renameSync(_logFile, `${_logFile}.${timestamp2}`);
      const files = fs$j.readdirSync(LOG_DIR).filter((f) => f.startsWith("app-") && f !== path$n.basename(_logFile)).sort().reverse();
      for (const file2 of files.slice(MAX_LOG_FILES)) {
        fs$j.unlinkSync(path$n.join(LOG_DIR, file2));
      }
    }
  } catch {
  }
...
function writeLog(entry) {
  ...
  try {
    ...
    fs$j.appendFileSync(_logFile, line, "utf-8");
    rotateLogs();
  } catch {
  }
}
...
function getSettingsPath() {
  return path$n.join(require$$1.app.getPath("userData"), SETTINGS_FILE);
}
function getStateDir() {
  const dir = path$n.join(require$$1.app.getPath("userData"), STATE_DIR);
...
function loadSettings() {
  try {
    ...
    if (fs$j.existsSync(filePath)) {
      const data = fs$j.readFileSync(filePath, "utf-8");
...
function saveSettings(settings) {
  try {
    const filePath = getSettingsPath();
    fs$j.writeFileSync(filePath, JSON.stringify(settings, null, 2), "utf-8");
  } catch (err) {
...
function saveLastSession(sessionId) {
  ...
  const filePath = path$n.join(stateDir, "last-session.json");
  try {
    fs$j.writeFileSync(filePath, JSON.stringify({ sessionId, timestamp: Date.now() }), "utf-8");
  } catch (err) {
    logWarn("persistence", "Failed to save last session", { error: String(err) });
  }
}
function saveCrashState(data) {
  const stateDir = getStateDir();
  const timestamp2 = Date.now();
  const filePath = path$n.join(stateDir, `crash-${timestamp2}.json`);
  try {
    fs$j.writeFileSync(filePath, JSON.stringify({ ...data, timestamp: timestamp2 }, null, 2), "utf-8");
    logInfo("crash", "Crash state saved", { path: filePath });
  } catch (err) {
    logWarn("crash", "Failed to save crash state", { error: String(err) });
  }
}
function getCrashInfo(error2, type2) {
  const message = typeof error2 === "string" ? error2 : error2.message;
  const stack = typeof error2 === "object" ? error2.stack : void 0;
  return {
    type: type2,
    message,
    stack,
    ...
    appVersion: require$$1.app.getVersion()
  };
}
function setupCrashReporting() {
  process.on("uncaughtException", (error2) => {
    const info = getCrashInfo(error2, "uncaughtException");
    logError("crash", "Uncaught exception", info);
    saveCrashState(info);
  });
  process.on("unhandledRejection", (reason) => {
    ...
    logError("crash", "Unhandled rejection", info);
    saveCrashState(info);
  });
...
function checkForCrashOnStartup() {
  const stateDir = path$n.join(require$$1.app.getPath("userData"), "state");
  try {
    if (!fs$j.existsSync(stateDir)) return false;
    const files = fs$j.readdirSync(stateDir).filter((f) => f.startsWith("crash-") && f.endsWith(".json")).sort().reverse();
    if (files.length === 0) return false;
    const latestCrash = path$n.join(stateDir, files[0]);
    const data = JSON.parse(fs$j.readFileSync(latestCrash, "utf-8"));
    const crashTime = data.timestamp ?? 0;
    const now = Date.now();
    if (now - crashTime < 24 * 60 * 60 * 1e3) {
      return true;
    }
    return false;
  } catch {
...
function setupUpdater(mainWindow2) {
  ...
  main$1.autoUpdater.on("error", (error2) => {
    ...
    _updateState.error = error2.message;
    logError("updater", "Update error", { error: error2.message });
    notifyRenderer("update-status", _updateState);
  });
}
...
function showUpdateNotification(info) {
  ...
  require$$1.dialog.showMessageBox(_mainWindow, {
    type: "info",
    ...
    if (response === 0) {
      main$1.autoUpdater.downloadUpdate().catch((err) => {
        logError("updater", "Download failed", { error: String(err) });
      });
    }
  });
}
function showRestartPrompt(info) {
  if (!_mainWindow || _mainWindow.isDestroyed()) return;
  require$$1.dialog.showMessageBox(_mainWindow, {
    type: "info",
    title: "Update Ready",
...
function addUpdaterMenu(menuTemplate) {
  return [
    ...menuTemplate,
    {
      label: "Help",
...
function exportDiagnostics() {
  ...
  require$$1.dialog.showOpenDialog(_mainWindow, {
...
function showAbout() {
  require$$1.dialog.showMessageBox(_mainWindow, {
    ...
    message: `Game Agent v${require$$1.app.getVersion()}`,
...
let mainWindow = null;
async function waitForUrl(url, maxRetries = 15, delayMs = 500) {
  for (let i = 0; i < maxRetries; i++) {
    try {
      const response = await fetch(url, { method: "HEAD", signal: AbortSignal.timeout(2e3) });
...
function createWindow() {
  ...
  const settings = loadSettings();
  const windowOptions = {
    width: ((_a = settings.windowBounds) == null ? void 0 : _a.width) ?? 1280,
    height: ((_b = settings.windowBounds) == null ? void 0 : _b.height) ?? 800,
    x: ((_c = settings.windowBounds) == null ? void 0 : _c.x) ?? void 0,
    y: ((_d = settings.windowBounds) == null ? void 0 : _d.y) ?? void 0,
    minWidth: 900,
    minHeight: 600,
    webPreferences: {
    ...
    title: "Game Agent",
    icon: path$n.join(__dirname, "../build/icon.ico"),
    show: false
  };
  const win = new require$$1.BrowserWindow(windowOptions);
...
async function loadWindowContent(win) {
  ...
  if (devUrl) {
    try {
      ...
      logWarn("app", `Vite dev server not ready after retries: ${String(error2)}`);
    }
    if (!win.isDestroyed()) {
    ...
    if (!win.isDestroyed()) {
      await win.loadFile(path$n.join(__dirname, "../dist/index.html"));
    }
  }
}
function buildMenu() {
  const template = [
    {
      label: "File",
      submenu: [
        { label: "New Session", accelerator: "CmdOrCtrl+N", click: () => mainWindow == null ? void 0 : mainWindow.webContents.send("menu-new-session") },
        { type: "separator" },
        { role: "quit" }
      ]
    },
  ...
  require$$1.Menu.setApplicationMenu(require$$1.Menu.buildFromTemplate(addUpdaterMenu(template)));
}
require$$1.app.whenReady().then(async () => {
  ...
  const settings = loadSettings();
  settings.lastVersion = require$$1.app.getVersion();
  settings.lastLaunch = (/* @__PURE__ */ new Date()).toISOString();
  saveSettings(settings);
