...
contextBridge.exposeInMainWorld("unoApi", {
  // Health & connection
  healthCheck: async (port: number) => {
    const r = await fetch(`${API_BASE}:${port}/health`, { signal: AbortSignal.timeout(3000) });
    if (!r.ok) {
  ...
  getReplayDetail: async (replayId: string) => {
  ...
  createSession: async (spec: Record<string, unknown>) => {
    const r = await fetch(`${ORCH}/sessions`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(spec) });
    if (!r.ok) throw new Error(await r.text());
    return r.json();
  },
  attachAdapter: async (sessionId: string, body: Record<string, unknown>) => {
    const timeoutMs = body.adapter_type === "web" ? 120_000 : 30_000;
  ...
  tickSession: async (sessionId: string) => {
  ...
  getSessionSteps: async (sessionId: string) => {
  ...
  // Profiles & health
  getProfileHealthSummary: async (profileId: string) => {
  ...
  getWindowsRpaPreview: async (adapterId: string) => {
